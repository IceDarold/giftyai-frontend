...
export const Wishlist: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-24 pb-32 px-4 bg-brand-dark relative overflow-hidden">
      
      {/* Background Decor */}
      <div className="fixed top-0 left-0 w-full h-96 bg-gradient-to-b from-brand-main/10 to-transparent -z-10" />
      <div className="fixed top-20 right-[-50px] w-64 h-64 bg-brand-purple/10 rounded-full blur-[80px] -z-10 animate-pulse-slow" />

      <div className="max-w-4xl mx-auto">
        
        {/* --- 1. IMMERSIVE HEADER (Wallet Card) --- */}
        <div className="relative mb-8 group perspective-1000">
            <div className="absolute inset-0 bg-gradient-to-r from-brand-main to-brand-purple blur-2xl opacity-30 rounded-[2.5rem] transform translate-y-4 scale-90 transition-all group-hover:scale-95 group-hover:opacity-40"></div>
            
            <div className="relative bg-gradient-to-br from-brand-main via-brand-purple to-brand-accent rounded-[2.5rem] p-8 md:p-10 text-white shadow-2xl overflow-hidden border border-white/10">
                {/* Texture */}
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
                    <div>
                        <p className="text-white/80 font-bold uppercase tracking-widest text-xs mb-1">Стоимость желаний</p>
        ...
        <div className="sticky top-6 z-40 mb-8">
            <div className="bg-white/80 backdrop-blur-xl rounded-[1.5rem] p-2 pl-4 shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)] border border-slate-100 flex items-center gap-2">
                ...
                <div className="flex-grow flex items-center gap-3">
                    ...
                    <input 
                        ...
                        className="w-full bg-transparent outline-none text-slate-900 font-bold placeholder-slate-400 h-10"
                    />
                </div>
                ...
                <button 
                    ...
                    className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${isSelectionMode ? 'bg-slate-900 text-white shadow-lg' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                >
        ...
        {loading ? (
            <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="text-slate-400 font-medium animate-pulse">Загружаем желания...</p>
            </div>
        ) : items.length === 0 ? (
            // EMPTY STATE
            <div className="flex flex-col items-center text-center py-10 animate-pop">
                <div className="relative mb-6">
                    <div className="absolute inset-0 bg-brand-yellow/30 blur-[60px] rounded-full"></div>
                    <Mascot emotion="thinking" className="w-40 h-40 relative z-10" accessory="none" />
                </div>
                <h2 className="text-2xl font-black text-slate-900 mb-2">Вишлист пуст</h2>
                ...
                {filteredItems.map((gift) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Selection Overlay */}
                            {isSelectionMode && (
                                <div className={`absolute inset-0 z-20 rounded-[20px] transition-all duration-200 border-[3px] flex items-start justify-end p-3 ${isSelected ? 'bg-brand-main/10 border-brand-main' : 'bg-white/50 border-transparent hover:bg-white/30'}`}>
                                    <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${isSelected ? 'bg-brand-main border-brand-main scale-110' : 'bg-white border-slate-300'}`}>
                                        {isSelected && (
      ...
      <div className={`fixed bottom-6 left-4 right-4 md:left-1/2 md:-translate-x-1/2 md:max-w-md z-50 transition-all duration-500 ${isSelectionMode && selectedItems.size > 0 ? 'translate-y-0 opacity-100' : 'translate-y-[150%] opacity-0'}`}>
          <div className="bg-white rounded-2xl p-4 shadow-2xl flex items-center justify-between border border-slate-100">
              <div className="font-bold pl-2 text-slate-800">
                  Выбрано: {selectedItems.size}
              </div>
              <div className="flex gap-2">
                  <button 
                    onClick={handleBulkRemove}
                    className="bg-brand-love/10 hover:bg-brand-love text-brand-love hover:text-white px-4 py-2 rounded-xl font-bold text-sm transition-colors"
                  >
                      Удалить
                  </button>
                  <button 
                    onClick={handleShare}
                    className="bg-brand-main text-white hover:brightness-110 px-4 py-2 rounded-xl font-bold text-sm transition-colors shadow-md"
                  >
                      Поделиться
                  </button>
              </div>
