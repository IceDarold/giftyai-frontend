...
export const GiftDetailsModal: React.FC<Props> = ({ gift: initialGift, answers, isOpen, onClose, onWishlistChange, onUpdate }) => {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center pointer-events-none">
      <div className={`absolute inset-0 bg-slate-900/40 backdrop-blur-xl pointer-events-auto transition-opacity duration-500 ${isOpen ? 'opacity-100' : 'opacity-0'}`} onClick={onClose} />
      <div className={`bg-white w-full max-w-2xl h-[96vh] sm:h-[90vh] sm:max-h-[850px] rounded-t-[3rem] sm:rounded-[3rem] relative z-10 flex flex-col pointer-events-auto shadow-2xl transform transition-transform duration-500 overflow-hidden ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        ...
        <div className="absolute top-6 right-6 z-40 flex gap-2">
            {isDevMode && (
                <button 
                    ...
                    className={`h-11 px-4 rounded-full flex items-center justify-center font-bold text-sm backdrop-blur-md transition-all ${isEditing ? 'bg-brand-accent text-white' : 'bg-white/20 hover:bg-white/40 text-slate-800 shadow-sm border border-slate-200'}`}
                >
                    {isEditing ? 'Done' : 'Edit'}
                </button>
            )}
            <button onClick={onClose} className="w-11 h-11 bg-white/20 hover:bg-white/40 backdrop-blur-md rounded-full flex items-center justify-center text-slate-800 shadow-sm border border-slate-200"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>

        <div ref={contentRef} className="flex-grow overflow-y-auto no-scrollbar pb-32 bg-white">
           <div className="relative h-[50vh] sm:h-[450px] w-full shrink-0 group/image">
              ...
              {isEditing && (
                  <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center p-6 gap-4">
                      ...
                      <label className="bg-brand-main hover:brightness-110 text-white px-4 py-2 rounded cursor-pointer">
                          Upload File
              ...
              <div className={`absolute inset-0 bg-gradient-to-b from-black/10 via-transparent to-white ${isEditing ? 'hidden' : ''}`} />
              
              {answers && matchScore > 0 && !isEditing && (
                <div className="absolute bottom-6 right-6 bg-white/90 backdrop-blur-xl p-2 pr-4 rounded-2xl shadow-xl flex items-center gap-3 border border-slate-100">
                    <div className="text-brand-main font-black">{matchScore}%</div>
           ...
           <div className="px-6 sm:px-10 -mt-6 relative z-10">
              ...
              <div className="relative overflow-hidden rounded-[2rem] p-6 mb-8 shadow-sm bg-brand-main/5 border border-brand-main/10">
                 {isEditing ? (
                     <textarea 
                        className="w-full bg-white border border-slate-200 rounded p-2 text-lg font-medium h-24"
                        value={gift.reason || ''}
                        onChange={e => handleEditChange('reason', e.target.value)}
                        placeholder="AI Reason"
                     />
                 ) : (
                     <p className="text-lg text-slate-700 font-medium italic">"{gift.reason || 'Отличный выбор от ИИ-Санты'}"</p>
                 )}
              </div>
              ...
              <div className="mb-10 text-slate-600 leading-relaxed">
                  {isEditing ? (
                      <textarea 
                        className="w-full bg-white border border-slate-200 rounded p-2 h-40"
        ...
        <div className="absolute bottom-0 left-0 right-0 z-50 p-6 bg-gradient-to-t from-white to-transparent">
            <div className="max-w-xl mx-auto flex items-center gap-4">
                 <button onClick={handleWishlist} className={`w-[4.5rem] h-[4.5rem] rounded-[1.5rem] flex items-center justify-center border transition-all ${saved ? 'bg-brand-love/10 border-brand-love/20 text-brand-love' : 'bg-white border-slate-200 text-slate-400 shadow-sm'}`}>
