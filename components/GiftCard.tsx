...
export const GiftCard: React.FC<Props> = ({ gift, featured = false, onToggleWishlist, onClick, rank = 0 }) => {
  ...
  return (
    <div 
      ...
      role="button"
      tabIndex={0}
      className={`group relative bg-white rounded-[20px] shadow-sm hover:shadow-xl transition-all duration-300 cursor-pointer flex flex-col h-full overflow-hidden border border-gray-100/50 outline-none focus:ring-4 focus:ring-brand-main/20 ${featured ? 'ring-2 ring-brand-main ring-offset-2' : ''}`}
    >
      {featured && (
        <div className="absolute top-0 left-0 z-20 bg-brand-main text-white text-[10px] font-bold px-2 py-1 rounded-br-lg shadow-sm">
          Лидер
        </div>
      )}
      ...
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-gray-50 rounded-t-[20px]">
        ...
        <button 
          ...
          className={`absolute top-2 right-2 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-200 z-10 shadow-sm outline-none focus:ring-2 focus:ring-brand-main ${saved ? 'bg-white/95 text-brand-main' : 'bg-white/70 hover:bg-white text-gray-400 hover:text-gray-600'}`}
        >
        ...
        {gift.merchant && (
          <div className="absolute bottom-2 right-2 flex justify-end items-end">
                <div className="bg-brand-main/90 backdrop-blur-md text-white text-[9px] font-bold px-1.5 py-0.5 rounded-[6px] shadow-sm flex items-center gap-1">
                    <span>✨</span>
                    {gift.merchant}
                </div>
          </div>
      ...
      <div className="p-3 flex flex-col flex-grow relative">
        ...
        <div className="flex items-center gap-1.5 mt-auto">
            <div className="flex items-center gap-0.5">
                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-brand-main fill-current" viewBox="0 0 20 20">
        ...
        <div className="absolute bottom-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity duration-200 lg:block hidden">
            <div className="bg-brand-main text-white w-8 h-8 rounded-full flex items-center justify-center shadow-md hover:brightness-110">
